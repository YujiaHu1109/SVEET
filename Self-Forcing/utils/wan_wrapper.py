...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        model_dir = os.environ.get("SVEET_MODEL_DIR")
        if not model_dir:
            raise RuntimeError("Set SVEET_MODEL_DIR to the merged Wan/VACE model directory.")
        text_encoder_path = os.environ.get(
            "SVEET_TEXT_ENCODER_PATH",
            os.path.join(model_dir, "models_t5_umt5-xxl-enc-bf16.pth"),
        )
        tokenizer_path = os.environ.get(
            "SVEET_TOKENIZER_PATH",
            os.path.join(model_dir, "google", "umt5-xxl"),
        )
        self.text_encoder.load_state_dict(torch.load(
            text_encoder_path, map_location="cpu", weights_only=False
        ))
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        vae_path = os.environ.get(
            "SVEET_VAE_PATH", os.path.join(model_dir, "Wan2.1_VAE.pth")
        )
...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        if vace_context is not None:
            vace_tmp = vace_context.permute(0, 2, 1, 3, 4) 
        else:
            vace_tmp = None
        # X0 prediction
