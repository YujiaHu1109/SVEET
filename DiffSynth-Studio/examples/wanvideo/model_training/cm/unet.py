...
class QKVFlashAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert batch_first, "FlashAttention only supports batch_first=True"
        super().__init__()
        ...
        assert self.head_dim in [16, 32, 64], \
            "FlashAttention only supports head_dim == 16, 32, or 64"
    ...
    def forward(
        ...
        qkv = rearrange(
            qkv,
            "b (three h d) s -> b s three h d",
            three=3,
            h=self.num_heads,
        )

        # FlashAttention 2.x 要求 contiguous
        qkv = qkv.contiguous()
        ...
        # [B, S, H, D] -> [B, H*D, S]
        out = rearrange(out, "b s h d -> b (h d) s")
        return out
