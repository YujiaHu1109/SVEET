...
from ..models.wan_video_dit_causal import DiTKVCache
from ..models.wan_video_dit_s2v import rope_precompute
from ..models.wan_video_text_encoder import WanTextEncoder, T5RelativeEmbedding, T5LayerNorm
from ..models.wan_video_vae import WanVideoVAE, RMS_norm, CausalConv3d, Upsample
from ..models.wan_video_image_encoder import WanImageEncoder
from ..models.wan_video_vace_causal import VaceWanModel_causal
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.vace: VaceWanModel_causal = None
        self.vace2: VaceWanModel_causal = None
    ...
    def cm_forward(self, timestep, **inputs):
        model_instances = {
            ...
            "vace": self.vace,
            "vap": self.vap,
            "animate_adapter": self.animate_adapter,
        }
        inputs.update(model_instances)
    ...
    def from_pretrained(
        ...
        vace = model_manager.fetch_model("wan_video_vace_causal", index=2)
    ...
    def __call__(
        ...
        ### newly added for autoregressive generation ###
        autoregressive_mode: bool = False,
        target_latent_num: int = None,  
        kv_cache_posi: Optional[torch.nn.Module] = None,  
        kv_cache_nega: Optional[torch.nn.Module] = None,
        ...
        **kwargs,
        ...
        inputs_shared = {
            ...
            "target_latent_num": target_latent_num,   
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if kv_cache_posi is not None and kv_cache_nega is not None:
                    kv_cache_posi.set_step(progress_id)
                    kv_cache_nega.set_step(progress_id)

            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, kv_cache=kv_cache_posi)
            if cfg_scale != 1.0:
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
                    noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep, kv_cache=kv_cache_nega)
        ...
        if autoregressive_mode:
            ...
            target_original_frames = inputs_shared["num_frames"]
            if target_original_frames == 1:
                
                pass
            elif target_original_frames == 4:
                
                
                latents = torch.cat([latents, latents], dim=2)  
            else:
                
                
                required_latent_num = (target_original_frames // 4) + 2
                latents = torch.cat([latents] * required_latent_num, dim=2)
        ...
        if autoregressive_mode:
            if target_original_frames == 4:
                
                video = video[:, :, -4:, :, :]
                
            elif 1 < target_original_frames < 4:
                
                video = video[:, :, -target_original_frames:, :, :]
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    def __init__(self):
        ...
        super().__init__(input_params=("height", "width", "num_frames", "autoregressive_mode"))

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, autoregressive_mode=False):
        if autoregressive_mode:
            height = ((height + pipe.height_division_factor - 1) // pipe.height_division_factor) * pipe.height_division_factor
            width = ((width + pipe.width_division_factor - 1) // pipe.width_division_factor) * pipe.width_division_factor
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image", "autoregressive_mode", "target_latent_num"))

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image, autoregressive_mode=False, target_latent_num=None):
        if autoregressive_mode:
            length = target_latent_num if target_latent_num is not None else 1
        else:
            length = (num_frames - 1) // 4 + 1
            if vace_reference_image is not None:
                f = len(vace_reference_image) if isinstance(vace_reference_image, list) else 1
        ...
        shape = (1, pipe.vae.model.z_dim, length, latent_h, latent_w)
        noise = pipe.generate_noise(shape, seed=seed, rand_device=rand_device)
        
        if vace_reference_image is not None and not autoregressive_mode:
            f = len(vace_reference_image) if isinstance(vace_reference_image, list) else 1
            noise = torch.concat((noise[:, :, -f:], noise[:, :, :-f]), dim=2)
        return {"noise": noise}
...
def model_fn_wan_video(
    ...
    kv_cache: Optional[DiTKVCache] = None,
    latent_idx: int = None,
    **kwargs,
):
    ...
    if latent_idx is not None:
        ...
        t_freqs = dit.freqs[0][latent_idx : latent_idx + f].view(f, 1, 1, -1)
        
        
        h_freqs = dit.freqs[1][:h].view(1, h, 1, -1)
        w_freqs = dit.freqs[2][:w].view(1, 1, w, -1)
        
        
        
        freqs = torch.cat([
            t_freqs.expand(f, h, w, -1),
            h_freqs.expand(f, h, w, -1),
            w_freqs.expand(f, h, w, -1)
    ...
    if tea_cache_update:
        ...
        # def create_custom_forward(module, layer_id, kv_cache):
        #     def custom_forward(*inputs):
        #         # inputs = (x, context, t_mod, freqs)
        #         return module(*inputs, kv_cache=kv_cache)
        #     return custom_forward
        ...
        for block_id, block in enumerate(dit.blocks):
            if hasattr(block, 'self_attn') and hasattr(block.self_attn, 'layer_id'):
                block.self_attn.layer_id = block_id  
            # Block
                ...
                if use_gradient_checkpointing_offload:
                    with torch.autograd.graph.save_on_cpu():
                        x = torch.utils.checkpoint.checkpoint(
                            ...
                            # create_custom_forward(block, block_id, kv_cache),
                    ...
                    # x = block(x, context, t_mod, freqs, kv_cache=kv_cache)
...
@torch.no_grad()
def feature_extraction_fn(
    dit,
    latents: torch.Tensor,
    context: torch.Tensor,
    target_layers: list,
    timestep_value: int = 0,
):
    ...
    def make_hook(ell):
        def hook(module, inputs, output):
            ...
            feats[ell] = h.detach().to(torch.float32).cpu()
        return hook
    
    handles = []
    for ell in target_layers:
        handles.append(dit.blocks[ell].register_forward_hook(make_hook(ell)))
    
    try:
        # ---- 2. Timestep embedding (t=timestep_value) ----
        timestep = torch.full(
            (batch_size,), timestep_value,
            dtype=torch.long, device=device
        )
        t = dit.time_embedding(
            sinusoidal_embedding_1d(dit.freq_dim, timestep).to(dtype)
        )
        t_mod = dit.time_projection(t).unflatten(1, (6, dit.dim))
        
        # ---- 3. Text context embedding ----
        context = dit.text_embedding(context)
        
        # ---- 4. Patchify latents ----
        x = dit.patchify(latents, None)
        f, h, w = x.shape[2:]
        x = rearrange(x, 'b c f h w -> b (f h w) c').contiguous()
